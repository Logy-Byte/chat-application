...
/// Looks for a locally built library in `rust/target/<profile>/` or
/// `rust/target/<triple>/<profile>/` (or `rust/target_android/<triple>/<profile>/`).
///
...
Uri? _findLocalBuild(Uri packageRoot, OS targetOS, Architecture targetArch) {
  ...
  final candidateSubdirs = <String>[];

  // 1. Android cross-compilation targets (cargo-ndk / rustc)
  if (targetOS == OS.android) {
    final triple = _androidTriple(targetArch);
    if (triple != null) {
      for (final profile in ['release', 'debug']) {
        candidateSubdirs.add('rust/target_android/$triple/$profile/$fileName');
        candidateSubdirs.add('rust/target/$triple/$profile/$fileName');
      }
    }
  }

  // 2. Host-matching local build
  if (targetOS == OS.current && targetArch == Architecture.current) {
    for (final profile in ['release', 'debug']) {
      candidateSubdirs.add('rust/target/$profile/$fileName');
    }
  }

  for (final relPath in candidateSubdirs) {
    final path = packageRoot.resolve(relPath);
...
/// Returns the Rust target triple for the given Android architecture.
