...
fn acquire_single_writer_lock(db_path: &str) -> Result<Option<std::fs::File>, String> {
    ...
    loop {
        match try_lock_exclusive(&file) {
            ...
            Ok(false) => return Err(in_use_error()),
            Err(e) if e.kind() == std::io::ErrorKind::Unsupported => {
                // Some Android kernel / filesystem mount configurations do not support advisory flock.
                // SQLite's own internal lock handles concurrency in that scenario.
                return Ok(Some(file));
            }
            Err(e) => {
...
/// Android deliberately reports `Unsupported` for `std::fs::File::try_lock`.
/// Use fs4's safe `flock` wrapper there; keep the standard-library API on the
/// Unix targets where it is supported. `Ok(false)` means another engine owns
/// the sidecar lock.
