...
void main() {
  group('Chaty UI Template System Tests', () {
    ...
    test(
      'All 6 original templates exist in registry with non-null components',
      () {
    ...
    test(
      'Navigation destinations have zero duplicates within any template',
      () {
        for (final tmpl in ChatyTemplateRegistry.list) {
    ...
    test('Applying full template resets component overrides', () async {
      ...
      expect(
        controller.config.isOverridden(TemplateComponentType.navigation),
        isTrue,
      );
    ...
    test(
      'Component-level override modifies ONLY the targeted component',
      () async {
        await controller.applyFullTemplate(ChatyTemplateId.messageFirst);

        final beforeComposer = controller.resolveTemplateFor(
          TemplateComponentType.composer,
        );
        final beforeProfile = controller.resolveTemplateFor(
          TemplateComponentType.profile,
        );
        final beforeHome = controller.resolveTemplateFor(
          TemplateComponentType.home,
        );
        ...
        expect(
          controller.resolveTemplateFor(TemplateComponentType.profile),
          equals(beforeProfile),
        );
        expect(
          controller.resolveTemplateFor(TemplateComponentType.home),
          equals(beforeHome),
        );
      },
    );
    ...
    test(
      'Resetting a single component falls back to the active base template',
      () async {
        ...
        await controller.removeComponentOverride(
          TemplateComponentType.navigation,
        );
        expect(
          controller.resolveTemplateFor(TemplateComponentType.navigation),
    ...
    test('Serialization and deserialization round-trips correctly', () {
      ...
      expect(
        restored.resolveFor(TemplateComponentType.profile),
        equals(ChatyTemplateId.powerChat),
      );
    });
    ...
    test('Corrupted map input falls back safely without crashing', () {
      ...
      final invalidKeys = UserTemplateConfiguration.fromMap({
        ...
        'overrides': {'invalid_component': 'random_string'},
      });
