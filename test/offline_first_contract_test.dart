...
void main() {
  test('backend restores encrypted snapshots before remote refresh', () {
    ...
    expect(
      backend,
      contains('unawaited(_refreshAuthenticatedSession(session))'),
    );
  ...
  test(
    'chat timeline renders cache first and always positions newest message',
    () {
      final chat = File(
      ...
      expect(
        chat,
        contains('final cachedMessages = widget.dataStore.getMessages'),
      );
  ...
  test('startup does not wait for noncritical housekeeping', () {
    ...
    final deferredIndex = main.indexOf(
    ...
    expect(
      main,
      contains('Future<void> _initializeDeferredPlatformServices() async'),
    );
    expect(main, contains('await locator<AppIconController>().initialize();'));
    expect(
      main,
      contains('await locator<NotificationChannelManager>().initialize();'),
    );
